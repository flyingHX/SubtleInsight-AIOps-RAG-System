...
# ------------------ Service Layer ------------------
class Kb_change_setsService:
    """Service layer for Kb_change_sets operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Kb_change_sets]:
        """Create a new kb_change_sets"""
        try:
            obj = Kb_change_sets(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created kb_change_sets with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating kb_change_sets: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Kb_change_sets]:
        """Get kb_change_sets by ID"""
        try:
            query = select(Kb_change_sets).where(Kb_change_sets.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching kb_change_sets {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of kb_change_setss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Kb_change_sets, field):
                        column = getattr(Kb_change_sets, field)
            ...
            stmt = select(Kb_change_sets, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Kb_change_sets, field_name):
                        stmt = stmt.order_by(getattr(Kb_change_sets, field_name).desc())
                else:
                    if hasattr(Kb_change_sets, sort):
                        stmt = stmt.order_by(getattr(Kb_change_sets, sort))
            else:
                stmt = stmt.order_by(Kb_change_sets.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Kb_change_sets)
                for condition in conditions:
            ...
            logger.error(f"Error fetching kb_change_sets list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Kb_change_sets]:
        """Update kb_change_sets"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Kb_change_sets {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated kb_change_sets {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating kb_change_sets {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete kb_change_sets"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Kb_change_sets {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted kb_change_sets {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting kb_change_sets {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Kb_change_sets]:
        """Get kb_change_sets by any field"""
        try:
            if not hasattr(Kb_change_sets, field_name):
                raise ValueError(f"Field {field_name} does not exist on Kb_change_sets")
            column = getattr(Kb_change_sets, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Kb_change_sets).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching kb_change_sets by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Kb_change_sets]:
        """Get list of kb_change_setss filtered by field"""
        try:
            if not hasattr(Kb_change_sets, field_name):
                raise ValueError(f"Field {field_name} does not exist on Kb_change_sets")
            column = getattr(Kb_change_sets, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Kb_change_sets)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Kb_change_sets.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching kb_change_setss by {field_name}: {str(e)}")
            raise