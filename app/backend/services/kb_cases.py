...
# ------------------ Service Layer ------------------
class Kb_casesService:
    """Service layer for Kb_cases operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Kb_cases]:
        ...
        try:
            ...
            logger.info(f"Created kb_cases with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating kb_cases: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Kb_cases]:
        """Get kb_cases by ID"""
        try:
            query = select(Kb_cases).where(Kb_cases.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching kb_cases {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of kb_casess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Kb_cases, field):
                        column = getattr(Kb_cases, field)
            ...
            stmt = select(Kb_cases, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Kb_cases, field_name):
                        stmt = stmt.order_by(getattr(Kb_cases, field_name).desc())
                else:
                    if hasattr(Kb_cases, sort):
                        stmt = stmt.order_by(getattr(Kb_cases, sort))
            else:
                stmt = stmt.order_by(Kb_cases.id.desc())
            ...
            logger.error(f"Error fetching kb_cases list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Kb_cases]:
        """Update kb_cases"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Kb_cases {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated kb_cases {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating kb_cases {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Kb_cases {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted kb_cases {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting kb_cases {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Kb_cases]:
        """Get kb_cases by any field"""
        try:
            if not hasattr(Kb_cases, field_name):
                raise ValueError(f"Field {field_name} does not exist on Kb_cases")
            column = getattr(Kb_cases, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Kb_cases).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching kb_cases by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Kb_cases]:
        """Get list of kb_casess filtered by field"""
        try:
            if not hasattr(Kb_cases, field_name):
                raise ValueError(f"Field {field_name} does not exist on Kb_cases")
            column = getattr(Kb_cases, field_name)
            ...
            logger.error(f"Error fetching kb_casess by {field_name}: {str(e)}")
            raise