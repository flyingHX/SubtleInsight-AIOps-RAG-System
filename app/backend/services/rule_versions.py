...
# ------------------ Service Layer ------------------
class Rule_versionsService:
    """Service layer for Rule_versions operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Rule_versions]:
        """Create a new rule_versions"""
        try:
            obj = Rule_versions(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created rule_versions with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating rule_versions: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Rule_versions]:
        """Get rule_versions by ID"""
        try:
            query = select(Rule_versions).where(Rule_versions.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching rule_versions {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of rule_versionss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Rule_versions, field):
                        column = getattr(Rule_versions, field)
            ...
            stmt = select(Rule_versions, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Rule_versions, field_name):
                        stmt = stmt.order_by(getattr(Rule_versions, field_name).desc())
                else:
                    if hasattr(Rule_versions, sort):
                        stmt = stmt.order_by(getattr(Rule_versions, sort))
            else:
                stmt = stmt.order_by(Rule_versions.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Rule_versions)
                for condition in conditions:
            ...
            logger.error(f"Error fetching rule_versions list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Rule_versions]:
        """Update rule_versions"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Rule_versions {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated rule_versions {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating rule_versions {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Rule_versions {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted rule_versions {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting rule_versions {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Rule_versions]:
        """Get rule_versions by any field"""
        try:
            if not hasattr(Rule_versions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Rule_versions")
            column = getattr(Rule_versions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Rule_versions).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching rule_versions by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Rule_versions]:
        """Get list of rule_versionss filtered by field"""
        try:
            if not hasattr(Rule_versions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Rule_versions")
            column = getattr(Rule_versions, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Rule_versions.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching rule_versionss by {field_name}: {str(e)}")
            raise