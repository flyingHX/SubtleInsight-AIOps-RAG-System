...
# ------------------ Service Layer ------------------
class Approval_requestsService:
    """Service layer for Approval_requests operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Approval_requests]:
        """Create a new approval_requests"""
        try:
            obj = Approval_requests(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created approval_requests with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating approval_requests: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Approval_requests]:
        """Get approval_requests by ID"""
        try:
            query = select(Approval_requests).where(Approval_requests.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching approval_requests {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of approval_requestss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Approval_requests, field):
                        column = getattr(Approval_requests, field)
            ...
            stmt = select(Approval_requests, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Approval_requests, field_name):
                        stmt = stmt.order_by(getattr(Approval_requests, field_name).desc())
                else:
                    if hasattr(Approval_requests, sort):
                        stmt = stmt.order_by(getattr(Approval_requests, sort))
            else:
                stmt = stmt.order_by(Approval_requests.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Approval_requests)
                for condition in conditions:
            ...
            logger.error(f"Error fetching approval_requests list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Approval_requests]:
        """Update approval_requests"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Approval_requests {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated approval_requests {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating approval_requests {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Approval_requests {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted approval_requests {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting approval_requests {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Approval_requests]:
        """Get approval_requests by any field"""
        try:
            if not hasattr(Approval_requests, field_name):
                raise ValueError(f"Field {field_name} does not exist on Approval_requests")
            column = getattr(Approval_requests, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Approval_requests).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching approval_requests by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Approval_requests]:
        """Get list of approval_requestss filtered by field"""
        try:
            if not hasattr(Approval_requests, field_name):
                raise ValueError(f"Field {field_name} does not exist on Approval_requests")
            column = getattr(Approval_requests, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Approval_requests.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching approval_requestss by {field_name}: {str(e)}")
            raise