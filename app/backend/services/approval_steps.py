...
# ------------------ Service Layer ------------------
class Approval_stepsService:
    """Service layer for Approval_steps operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Approval_steps]:
        """Create a new approval_steps"""
        try:
            obj = Approval_steps(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created approval_steps with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating approval_steps: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Approval_steps]:
        """Get approval_steps by ID"""
        try:
            query = select(Approval_steps).where(Approval_steps.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching approval_steps {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of approval_stepss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Approval_steps, field):
                        column = getattr(Approval_steps, field)
            ...
            stmt = select(Approval_steps, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Approval_steps, field_name):
                        stmt = stmt.order_by(getattr(Approval_steps, field_name).desc())
                else:
                    if hasattr(Approval_steps, sort):
                        stmt = stmt.order_by(getattr(Approval_steps, sort))
            else:
                stmt = stmt.order_by(Approval_steps.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Approval_steps)
                for condition in conditions:
            ...
            logger.error(f"Error fetching approval_steps list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Approval_steps]:
        """Update approval_steps"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Approval_steps {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated approval_steps {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating approval_steps {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete approval_steps"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Approval_steps {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted approval_steps {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting approval_steps {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Approval_steps]:
        """Get approval_steps by any field"""
        try:
            if not hasattr(Approval_steps, field_name):
                raise ValueError(f"Field {field_name} does not exist on Approval_steps")
            column = getattr(Approval_steps, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Approval_steps).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching approval_steps by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Approval_steps]:
        """Get list of approval_stepss filtered by field"""
        try:
            if not hasattr(Approval_steps, field_name):
                raise ValueError(f"Field {field_name} does not exist on Approval_steps")
            column = getattr(Approval_steps, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Approval_steps.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching approval_stepss by {field_name}: {str(e)}")
            raise