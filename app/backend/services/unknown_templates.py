...
# ------------------ Service Layer ------------------
class Unknown_templatesService:
    """Service layer for Unknown_templates operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Unknown_templates]:
        """Create a new unknown_templates"""
        try:
            obj = Unknown_templates(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created unknown_templates with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating unknown_templates: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Unknown_templates]:
        """Get unknown_templates by ID"""
        try:
            query = select(Unknown_templates).where(Unknown_templates.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching unknown_templates {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of unknown_templatess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Unknown_templates, field):
                        column = getattr(Unknown_templates, field)
            ...
            stmt = select(Unknown_templates, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Unknown_templates, field_name):
                        stmt = stmt.order_by(getattr(Unknown_templates, field_name).desc())
                else:
                    if hasattr(Unknown_templates, sort):
                        stmt = stmt.order_by(getattr(Unknown_templates, sort))
            else:
                stmt = stmt.order_by(Unknown_templates.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Unknown_templates)
                for condition in conditions:
            ...
            logger.error(f"Error fetching unknown_templates list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Unknown_templates]:
        """Update unknown_templates"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Unknown_templates {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated unknown_templates {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating unknown_templates {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete unknown_templates"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Unknown_templates {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted unknown_templates {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting unknown_templates {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Unknown_templates]:
        """Get unknown_templates by any field"""
        try:
            if not hasattr(Unknown_templates, field_name):
                raise ValueError(f"Field {field_name} does not exist on Unknown_templates")
            column = getattr(Unknown_templates, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Unknown_templates).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching unknown_templates by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Unknown_templates]:
        """Get list of unknown_templatess filtered by field"""
        try:
            if not hasattr(Unknown_templates, field_name):
                raise ValueError(f"Field {field_name} does not exist on Unknown_templates")
            column = getattr(Unknown_templates, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Unknown_templates)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Unknown_templates.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching unknown_templatess by {field_name}: {str(e)}")
            raise