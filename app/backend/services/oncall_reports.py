...
# ------------------ Service Layer ------------------
class Oncall_reportsService:
    """Service layer for Oncall_reports operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Oncall_reports]:
        """Create a new oncall_reports"""
        try:
            obj = Oncall_reports(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created oncall_reports with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating oncall_reports: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Oncall_reports]:
        """Get oncall_reports by ID"""
        try:
            query = select(Oncall_reports).where(Oncall_reports.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching oncall_reports {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of oncall_reportss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Oncall_reports, field):
                        column = getattr(Oncall_reports, field)
            ...
            stmt = select(Oncall_reports, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Oncall_reports, field_name):
                        stmt = stmt.order_by(getattr(Oncall_reports, field_name).desc())
                else:
                    if hasattr(Oncall_reports, sort):
                        stmt = stmt.order_by(getattr(Oncall_reports, sort))
            else:
                stmt = stmt.order_by(Oncall_reports.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Oncall_reports)
                for condition in conditions:
            ...
            logger.error(f"Error fetching oncall_reports list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Oncall_reports]:
        """Update oncall_reports"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Oncall_reports {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated oncall_reports {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating oncall_reports {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete oncall_reports"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Oncall_reports {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted oncall_reports {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting oncall_reports {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Oncall_reports]:
        """Get oncall_reports by any field"""
        try:
            if not hasattr(Oncall_reports, field_name):
                raise ValueError(f"Field {field_name} does not exist on Oncall_reports")
            column = getattr(Oncall_reports, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Oncall_reports).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching oncall_reports by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Oncall_reports]:
        """Get list of oncall_reportss filtered by field"""
        try:
            if not hasattr(Oncall_reports, field_name):
                raise ValueError(f"Field {field_name} does not exist on Oncall_reports")
            column = getattr(Oncall_reports, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Oncall_reports)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Oncall_reports.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching oncall_reportss by {field_name}: {str(e)}")
            raise