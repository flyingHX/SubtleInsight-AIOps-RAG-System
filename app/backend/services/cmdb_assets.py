...
# ------------------ Service Layer ------------------
class Cmdb_assetsService:
    """Service layer for Cmdb_assets operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Cmdb_assets]:
        """Create a new cmdb_assets"""
        try:
            obj = Cmdb_assets(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created cmdb_assets with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating cmdb_assets: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Cmdb_assets]:
        """Get cmdb_assets by ID"""
        try:
            query = select(Cmdb_assets).where(Cmdb_assets.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching cmdb_assets {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of cmdb_assetss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Cmdb_assets, field):
                        column = getattr(Cmdb_assets, field)
            ...
            stmt = select(Cmdb_assets, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Cmdb_assets, field_name):
                        stmt = stmt.order_by(getattr(Cmdb_assets, field_name).desc())
                else:
                    if hasattr(Cmdb_assets, sort):
                        stmt = stmt.order_by(getattr(Cmdb_assets, sort))
            else:
                stmt = stmt.order_by(Cmdb_assets.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Cmdb_assets)
                for condition in conditions:
            ...
            logger.error(f"Error fetching cmdb_assets list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Cmdb_assets]:
        """Update cmdb_assets"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Cmdb_assets {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated cmdb_assets {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating cmdb_assets {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Cmdb_assets {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted cmdb_assets {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting cmdb_assets {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Cmdb_assets]:
        """Get cmdb_assets by any field"""
        try:
            if not hasattr(Cmdb_assets, field_name):
                raise ValueError(f"Field {field_name} does not exist on Cmdb_assets")
            column = getattr(Cmdb_assets, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Cmdb_assets).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching cmdb_assets by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Cmdb_assets]:
        """Get list of cmdb_assetss filtered by field"""
        try:
            if not hasattr(Cmdb_assets, field_name):
                raise ValueError(f"Field {field_name} does not exist on Cmdb_assets")
            column = getattr(Cmdb_assets, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Cmdb_assets.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching cmdb_assetss by {field_name}: {str(e)}")
            raise