...
# ------------------ Service Layer ------------------
class Console_configsService:
    """Service layer for Console_configs operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Console_configs]:
        """Create a new console_configs"""
        try:
            obj = Console_configs(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created console_configs with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating console_configs: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Console_configs]:
        """Get console_configs by ID"""
        try:
            query = select(Console_configs).where(Console_configs.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching console_configs {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of console_configss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Console_configs, field):
                        column = getattr(Console_configs, field)
            ...
            stmt = select(Console_configs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Console_configs, field_name):
                        stmt = stmt.order_by(getattr(Console_configs, field_name).desc())
                else:
                    if hasattr(Console_configs, sort):
                        stmt = stmt.order_by(getattr(Console_configs, sort))
            else:
                stmt = stmt.order_by(Console_configs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Console_configs)
                for condition in conditions:
            ...
            logger.error(f"Error fetching console_configs list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Console_configs]:
        """Update console_configs"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Console_configs {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated console_configs {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating console_configs {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete console_configs"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Console_configs {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted console_configs {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting console_configs {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Console_configs]:
        """Get console_configs by any field"""
        try:
            if not hasattr(Console_configs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Console_configs")
            column = getattr(Console_configs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Console_configs).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching console_configs by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Console_configs]:
        """Get list of console_configss filtered by field"""
        try:
            if not hasattr(Console_configs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Console_configs")
            column = getattr(Console_configs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Console_configs)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Console_configs.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching console_configss by {field_name}: {str(e)}")
            raise