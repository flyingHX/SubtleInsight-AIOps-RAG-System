...
class Agent_sessionsService:
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Agent_sessions]:
    ...
    async def get_by_id(self, obj_id: int) -> Optional[Agent_sessions]:
        """Get agent_sessions by ID"""
        try:
    ...
    async def get_list(
        ...
        """Get paginated list of agent_sessionss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Agent_sessions, field):
                        column = getattr(Agent_sessions, field)
            ...
            stmt = select(Agent_sessions, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Agent_sessions, field_name):
                        stmt = stmt.order_by(getattr(Agent_sessions, field_name).desc())
                else:
                    if hasattr(Agent_sessions, sort):
                        stmt = stmt.order_by(getattr(Agent_sessions, sort))
            else:
                stmt = stmt.order_by(Agent_sessions.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Agent_sessions)
                for condition in conditions:
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Agent_sessions]:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Agent_sessions]:
        ...
        try:
            ...
            column = getattr(Agent_sessions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Agent_sessions).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
