...
# ------------------ Service Layer ------------------
class Kb_merge_proposalsService:
    """Service layer for Kb_merge_proposals operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Kb_merge_proposals]:
        """Create a new kb_merge_proposals"""
        try:
            obj = Kb_merge_proposals(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created kb_merge_proposals with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating kb_merge_proposals: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Kb_merge_proposals]:
        """Get kb_merge_proposals by ID"""
        try:
            query = select(Kb_merge_proposals).where(Kb_merge_proposals.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching kb_merge_proposals {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of kb_merge_proposalss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Kb_merge_proposals, field):
                        column = getattr(Kb_merge_proposals, field)
            ...
            stmt = select(Kb_merge_proposals, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Kb_merge_proposals, field_name):
                        stmt = stmt.order_by(getattr(Kb_merge_proposals, field_name).desc())
                else:
                    if hasattr(Kb_merge_proposals, sort):
                        stmt = stmt.order_by(getattr(Kb_merge_proposals, sort))
            else:
                stmt = stmt.order_by(Kb_merge_proposals.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Kb_merge_proposals)
                for condition in conditions:
            ...
            logger.error(f"Error fetching kb_merge_proposals list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Kb_merge_proposals]:
        """Update kb_merge_proposals"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Kb_merge_proposals {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated kb_merge_proposals {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating kb_merge_proposals {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete kb_merge_proposals"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Kb_merge_proposals {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted kb_merge_proposals {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting kb_merge_proposals {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Kb_merge_proposals]:
        """Get kb_merge_proposals by any field"""
        try:
            if not hasattr(Kb_merge_proposals, field_name):
                raise ValueError(f"Field {field_name} does not exist on Kb_merge_proposals")
            column = getattr(Kb_merge_proposals, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Kb_merge_proposals).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching kb_merge_proposals by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Kb_merge_proposals]:
        """Get list of kb_merge_proposalss filtered by field"""
        try:
            if not hasattr(Kb_merge_proposals, field_name):
                raise ValueError(f"Field {field_name} does not exist on Kb_merge_proposals")
            column = getattr(Kb_merge_proposals, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Kb_merge_proposals)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Kb_merge_proposals.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching kb_merge_proposalss by {field_name}: {str(e)}")
            raise